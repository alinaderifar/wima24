...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        if (chartData && chartData.length > 0) {
            ...
            document.getElementById('lineChartUsers').innerHTML = '<div class="text-muted">{{ __("admin.No data found") }}</div>';
        }
    } catch (error) {
        console.error('Error rendering lineChartUsers:', error);
        document.getElementById('lineChartUsers').innerHTML = '<div class="text-danger">{{ __("admin.Error loading chart") }}</div>';
    }
});
</script>
@endpush