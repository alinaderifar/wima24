...
/*
|--------------------------------------------------------------------------
| Optimized CityController + Blade (Single Page) for Gambia Data
|--------------------------------------------------------------------------
*/
...
class CityController extends Controller
{
    public function index(Request $request)
    {
        ...
        echo '<input type="text" name="search" value="' . ($search ?? '') . '" placeholder="????? ???..." class="px-3 py-2 border rounded w-full md:w-1/3">';
        echo '<button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded">?????</button>';
        echo '</form>';
        ...
        foreach ($cities as $city) {
            ...
            echo '<p>?????: ' . ($city->population ?? '-') . '</p>';
            echo '<p>??????: ' . ($city->latitude ?? '-') . ', ' . ($city->longitude ?? '-') . '</p>';
            echo '</div>';
        }
        echo '</div>';
